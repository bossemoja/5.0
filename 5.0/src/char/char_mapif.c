...
/**
* Map-serv request to save cart for vending
* @param fd: wich fd to parse from
* @param id: wich map_serv id
* @return : 0 not enough data received, 1 success
*/
int chmapif_parse_save_cart(int fd, int id){
	if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2))
		return 0;
	else {
		int aid = RFIFOL(fd, 4), cid = RFIFOL(fd, 8), size = RFIFOW(fd, 2);
			...
			character->char_id == cid)
		{
			...
			memcpy(cart, RFIFOP(fd, 12), sizeof(cart));

			if (!char_cart_to_sql(cart, MAX_CART, cid))
			{
				WFIFOHEAD(fd, size);
				memcpy((char*)WFIFOP(fd, 0), (char*)RFIFOP(fd, 0), 12);
				memcpy(WFIFOP(fd, 12), cart, sizeof(cart));
				WFIFOSET(fd, size);
			}
			else
				ShowError("parse_from_map (save-cart): sql is error\n", aid, cid);
		}
		else {	//This may be valid on char-server reconnection, when re-sending characters that already logged off.
			ShowError("parse_from_map (save-char): Received data for non-existant/offline character (%d:%d).\n", aid, cid);
			char_set_char_online(id, cid, aid);
...
int chmapif_parse(int fd){
	...
	while(RFIFOREST(fd) >= 2){
		...
		switch(RFIFOW(fd,0)){
			...
			case 0x2f00: next=chmapif_parse_save_cart(fd, id); break;
			default:
			{
					// inter server - packet
