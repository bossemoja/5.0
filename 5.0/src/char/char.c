...
/// Saves an array of 'item cart' entries into the specified table.
int char_cart_to_sql(struct item items[], int max, int id){
	StringBuf buf;
	SqlStmt* stmt;
	int i;
	int j;
	...
	for (i = 0; i < max; ++i)
	{
		...
		if (SQL_ERROR == Sql_QueryStr(sql_handle, StringBuf_Value(&buf)))
		{
			...
			items[i].id = (int)Sql_LastInsertId(sql_handle);
	}
