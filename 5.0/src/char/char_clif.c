...
int chclif_parse_char_delete2_req(int fd, struct char_session_data* sd) {
	...
	if( SQL_SUCCESS != Sql_Query(sql_handle, "SELECT `karma` FROM `%s` WHERE `char_id`='%d'", schema_config.char_db, char_id) || SQL_SUCCESS != Sql_NextRow(sql_handle) )
	{
		Sql_ShowDebug(sql_handle);
		chclif_char_delete2_ack(fd, char_id, 3, 0);
		return 1;
	...
	Sql_GetData(sql_handle, 0, &data, NULL); karma = strtoul(data, NULL, 10);
