...
/*==========================================
* Saves cart data. : pk 2f00
*------------------------------------------*/
int chrif_save_cart(struct map_session_data *sd) {
	uint16 len = 0;

	nullpo_retr(-1, sd);
	chrif_check(-1); //Character is saved on reconnect.

	len = sizeof(sd->status.cart) + 12;
	WFIFOHEAD(char_fd, len);
	WFIFOW(char_fd, 0) = 0x2f00;
	WFIFOW(char_fd, 2) = len;
	WFIFOL(char_fd, 4) = sd->status.account_id;
	WFIFOL(char_fd, 8) = sd->status.char_id;
	memcpy(WFIFOP(char_fd, 12), sd->status.cart, sizeof(sd->status.cart));
	WFIFOSET(char_fd, WFIFOW(char_fd, 2));
...
/*==========================================
* sync id of item cart data. : pk 2f00
*------------------------------------------*/
int chrif_parse_save_cart(int fd) {
	int aid = RFIFOL(fd, 4), cid = RFIFOL(fd, 8);
	struct map_session_data *sd;

	if ((sd = map_id2sd(aid)) == NULL || sd->status.char_id != cid)
		return 0;

	memcpy(sd->status.cart, (char *)RFIFOP(fd, 12), sizeof(sd->status.cart));
	sd->state.save_cart = 0;

	clif_displaymessage(sd->fd, "คุณสามารถตั้งร้านค้าได้แล้ว....");
