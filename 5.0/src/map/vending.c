...
int8 vending_openvending(struct map_session_data* sd, const char* message, const uint8* data, int count, struct s_autotrader *at)
{
	...
	for( j = 0; j < count; j++ ) {
		...
		if (sd->status.cart[sd->vending[i].index].id == 0) {
			...
			snprintf(msg, 256, "ไอเท็ม %s ยังไม่ได้บันทึก. กรุณาออกเข้าใหม่ เพื่อให้ไอเท็มทำการเซฟลงร้านค้า", item->jname);
	...
	for (n = 0; n < k; n++)
	{
		...
		if (battle_config.vending_price_min_overflow > 0)
			sprintf(msg, "%s ตั้งราคาขายสูงกว่าที่ NPC มีขายอยู่เกิน %d%% จะถูกตัดออกจากรายการเพื่อป้องกันการโก่งราคา", item->jname, battle_config.vending_price_min_overflow/100);
		else
			sprintf(msg, "%s ตั้งราคาขายสูงกว่าที่ NPC มีขายอยู่ จะถูกตัดออกจากรายการเพื่อป้องกันการโก่งราคา", item->jname);
		clif_displaymessage(sd->fd, msg);
	}
