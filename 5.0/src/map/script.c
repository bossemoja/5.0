...
BUILDIN_FUNC(explode)
{
	...
	// clear value of array size
	for (k = start; k < SCRIPT_MAX_ARRAYSIZE; k++)
		set_reg(st, sd, reference_uid(id, k), name, (void *)"", reference_getref(data));
...
BUILDIN_FUNC(setcharprotection)
{
	TBL_PC *sd = script_rid2sd(st);
	int var = script_getnum(st,2);
	if( sd == NULL ) return true;
		
	sd->state.account_protection = (var)?1:0;
	return 0;
}
...
BUILDIN_FUNC(enableemotiondice)
{
	battle_config.enable_emotion_dice = script_getnum(st, 2);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(reloadmobrate)
{
	mob_reload();
...
BUILDIN_FUNC(getaccountreg)
{
	TBL_PC *sd;

	if (!(sd = map_charid2sd(script_getnum(st, 2))))
	{
		script_pushint(st, -1);
		return SCRIPT_CMD_SUCCESS;
	}

	script_pushint(st, pc_readaccountreg(sd, script_getstr(st, 3)));
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(setaccountreg)
{
	TBL_PC *sd;
	...
	pc_setaccountreg(sd, script_getstr(st, 3), script_getnum(st, 4));
	return SCRIPT_CMD_SUCCESS;
}
...
struct script_function buildin_func[] = {
	// Custom
	BUILDIN_DEF(enableemotiondice, "i"),
	BUILDIN_DEF(reloadmobrate, ""),
	BUILDIN_DEF(getaccountreg, "is"),
	BUILDIN_DEF(setaccountreg, "isi"),
	// NPC interaction
	BUILDIN_DEF(mes,"s*"),
	BUILDIN_DEF(next,""),
	BUILDIN_DEF(close,""),
...
// <--- [zBuffer] List of mob control commands
	...
	BUILDIN_DEF(setcharprotection,"i"),
