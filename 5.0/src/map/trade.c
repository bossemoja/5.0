...
void trade_traderequest(struct map_session_data *sd, struct map_session_data *target_sd)
{
	...
	if( sd->state.account_protection ) {
		clif_displaymessage(sd->fd, "คุณไม่สามารถแลกเปลี่ยนได้โดย @Lock");
		return;
	}
	...
	if( target_sd->state.account_protection ) {
		clif_displaymessage(sd->fd, "ผู้เล่นอีกฝ่ายไม่สามาถแลกเปลี่ยนได้โดย @Lock");
		return;
	}
